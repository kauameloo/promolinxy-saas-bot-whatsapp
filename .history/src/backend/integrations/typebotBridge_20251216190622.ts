...
export class TypebotBridge {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      // Preservar sessões por padrão ao iniciar.
      // Se realmente precisar limpar tudo em um deploy específico, defina TYPEBOT_RESET_ON_STARTUP=true
      if (process.env.TYPEBOT_RESET_ON_STARTUP === "true") {
        await this.redisSession.clearAllSessions()
        console.log("[TypebotBridge] Reset de sessões habilitado (TYPEBOT_RESET_ON_STARTUP=true). Todas as sessões foram limpas.")
      } else {
        console.log("[TypebotBridge] Preservando sessões existentes no startup (TYPEBOT_RESET_ON_STARTUP != 'true')")
      }
